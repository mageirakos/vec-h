#!/usr/bin/env python3
"""Generate synthetic VSDS datasets for performance testing.

Creates a complete VSDS-compatible dataset directory at any scale factor,
with random vectors and realistic distributions fitted to actual SF1 Ind&Sci data.
Output is directly loadable by Maximus maxbench (--path <output_dir> --benchmark vsds).

Usage:
    # SF1 test (fast, ~1 min)
    uv run python3 scripts/generate_synthetic_vsds.py --sf 1

    # SF30 perf test (~15 min, ~34 GB RAM for TPC-H + streaming writes)
    uv run python3 scripts/generate_synthetic_vsds.py --sf 30

    # Custom dimensions, skip TPC-H if already generated
    uv run python3 scripts/generate_synthetic_vsds.py --sf 30 --rev_dim 256 --skip_tpch
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate synthetic VSDS dataset for performance testing."
    )
    ...
    print(f"=== Synthetic VSDS Generator ===")
    print(f"  SF:          {sf}")
